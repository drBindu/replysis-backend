...
public class SimpleRateLimiter {
    ...
    @Value("${security.trusted-proxy.enabled:false}")
    ...
    /**
     * Forwarded IP headers are trusted only when the deployment explicitly
     * enables them after placing this service behind a controlled reverse proxy.
     */
    public String clientIp(jakarta.servlet.http.HttpServletRequest request) {
        if (!trustForwardedHeaders) return request.getRemoteAddr();
